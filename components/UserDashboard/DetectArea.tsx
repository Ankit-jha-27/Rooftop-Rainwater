...
export function sqmToSqft(sqm: number | string): number {
  const n = Number(sqm);
...
const DetectArea: React.FC = () => {
  ...
  useEffect(() => {
    ...
    roofs.forEach(roof => {
      ...
      const areaLabel = L.divIcon({
        ...
        html: `<div>${sqmToSqft(roof.areaSqM).toFixed(2)} sqft</div><div class="text-[10px] text-gray-500">${Math.round(roof.areaSqM * 10.764)} sqft</div>`
      });
  ...
  const detectRoofWithGemini = async () => {
    ...
    try {
      ...
      // 4. Call Gemini
      const apiKey = "AIzaSyA13vmUePAoHZNJ5HCKlf6IHqh96XvCCkw";
              ...
              {roofs.map((roof, i) => (
                <div key={roof.id} className="flex justify-between items-center text-sm border-b border-slate-100 pb-1 last:border-0">
                  ...
                  <span className="font-mono font-medium">
                    {sqmToSqft(roof.areaSqM).toFixed(2)} sqft
                  </span>
                </div>
              ))}
