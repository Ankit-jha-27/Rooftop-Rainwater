...
const Assessment = ({ rainfall }: { rainfall: number }) => {
  ...
  const [feasibilityData, setFeasibilityData] = useState<boolean>(false);
  // User data state
  ...
  const checkOrCreateAssessment = React.useCallback(
    async (
      ...
      try {
        ...
        // ----------------------------------------
        // 1️⃣ CHECK IF ASSESSMENT ALREADY EXISTS
        // ----------------------------------------
        ...
        const existing = await getDocs(assessmentsQuery);
        let assessmentDocId: string;

        if (!existing.empty) {
          // Use existing doc
          const docSnap = existing.docs[0];
          assessmentDocId = docSnap.id;
          ...
          const newRef = doc(collection(firestore, "assessments"));
          assessmentDocId = newRef.id;
        ...
        // ------------------------------------------------------
        // 2️⃣ LISTEN FOR ASSESSMENT STATUS UPDATES (REALTIME)
        // ------------------------------------------------------
        ...
        // ------------------------------------------------------
        // 3️⃣ LISTEN FOR THE USER'S REPORT DOCUMENT
        // reports/<uid>
        // ------------------------------------------------------
        const reportRef = doc(firestore, "reports", uid);
        console.log("Checking report for UID:", uid);
        ...
        const globalTimeout = setTimeout(() => {
          ...
          setErrorMessage("Assessment taking too long. Please refresh.");
          setLoading(false);
        }, MAX_WAIT);
        ...
        const unsubscribeReport = onSnapshot(reportRef, (snapshot) => {
          ...
          // If report doesn't exist yet → keep waiting up to MAX_WAIT
          const elapsed = Date.now() - startTime;

          if (elapsed > MAX_WAIT) {
            clearTimeout(globalTimeout);
            setAssessmentStatus("error");
            setErrorMessage("Report timed out. Try again.");
            setLoading(false);
            return;
          }
        ...
        setAssessmentStatus("error");
        setErrorMessage("Could not create or load assessment.");
        setLoading(false);
      }
    },
    []
        ...
        const unsubscribeSnapshot = onSnapshot(docRef, async (snapshot) => {
          if (snapshot.exists()) {
            try {
              ...
              setFeasibilityData(data.feasibility);
              ...
              if (hasRequiredData && hasValidCoords && coords) {
                await checkOrCreateAssessment(currentUser.uid, {
                  name: data.fullName || data.username || "As sessment",
                  location: coords,
