...
import { getYearlyRainfall } from "@/utils/getFullYearRange";
...
const Page = () => {
  ...
  const [annualRainfall, setAnnualRainfall] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    const fetchRainfall = async () => {
      try {
        ...
        const result = await getYearlyRainfall(lat, lon, 2024)
        setAnnualRainfall(result.totalRainfallMM)
          ...
          {activeItem === "assessment" &&
            (hasRooftop ? (
              <>
                ...
                />
                <Assessment rainfall={annualRainfall} />
              </>
            ) : (
